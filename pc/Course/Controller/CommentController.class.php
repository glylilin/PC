...
class CommentController extends Controller
{
    ...
    public function moreList()
    {
        ...
        if (IS_AJAX) {
            ...
            $comment_service = D("Comment", 'Service');
            $comment_list = $comment_service->getCommentByRelIdService($id, $page);
            if ($comment_list['status']) {
                $res['status'] = true;
                foreach ($comment_list['data'] as $v) {
                    $res['data'] .= "<dd>".W('Comment/item', array($v))."</dd>";
                }
            }
        }
        $this->ajaxReturn($res);
